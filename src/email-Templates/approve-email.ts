...
<body>
  <div class="container">
    ...
    <p> purchase request with ID <span class="highlight">${requestDisplayId}</span> titled "<strong>${title}</strong>" has been Moved on  By ${name}. Below are the details of the request:</p>

   <!-- <p>Receiver Details<strong></strong>,</p> -->

     ${receiverInfo ? `<p>Receiver Name: <span class="highlight">${receiverInfo}</span></p>` : ''}
    ${accountInfo ? `<p>Receiver Account No: <span class="highlight">${accountInfo}</span></p>` : ''}
    ...
    <table class="details-table">
      ...
      <tr>
        <th>Quantity</th>
      <!--  <td>${quantity}</td> -->
        <td>${quantity !== null && quantity !== undefined ? quantity : 0} ${unit.unitName}</td>
      </tr>
      <tr>
